...
public abstract class MondrianOlap4jConnection implements OlapConnection {
        ...
        throws SQLException
    {
        ...
        Map<String, Object> dbpropsMap = null;
		if (dbpropsMaps.size() != 1) {

			for (Map<String, Object> dbMap : dbpropsMaps) {
				Set set=dbMap.keySet();
				Iterator it = set.iterator();
				while (it.hasNext()){
					String key=(String) it.next();
					String value=(String) dbMap.get(key);
					if("DataSourceInfo".equals(key)&&url.contains(value)){
						dbpropsMap = dbMap;
						System.out.println(">>>>>>>>>find the dbMap of url");
					}
				}
//				String catalog = (String) dbMap.get("Catalog");
//				if (url.contains(catalog)) {
//					dbpropsMap = dbMap;
//					System.out.println(dbMap.get("Head"));
//					break;
//				}
